...
public class LagerDao extends AbstractDao<Lager> {
    ...
    public List<Lager> getAll(int jahrId) {
        return select(findAllFromJahr, ps -> ps.setInt(1, jahrId), rs -> new Lager(rs.getInt("laid"),
                rs.getString("name"), rs.getString("thema"), rs.getDate("datumStart"), rs.getDate("datumStop")));
    }

    public Lager get(int lagerId) {
        return selectOne(find, ps -> ps.setInt(1, lagerId), rs -> new Lager(rs.getInt("laid"), rs.getString("name"),
                rs.getString("thema"), rs.getDate("datumStart"), rs.getDate("datumStop")));
    }
