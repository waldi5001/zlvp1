...
public class TeilnehmerDao extends AbstractDao<Teilnehmer> {
    ...
    public List<Teilnehmer> getAll(final int gruppe) {
        return select(findAllTeilnehmerFromGruppe, ps -> ps.setInt(1, gruppe),
                rs -> new Teilnehmer(rs.getInt("sttegrid"), Geschlecht.fromDbId(rs.getInt("geschlecht")),
                        rs.getInt("peid"), rs.getString("vorname"), rs.getString("nachname"), rs.getString("strasse"),
                        rs.getString("plz"), rs.getString("ort"), rs.getDate("gebDat"), rs.getString("handy"),
                        rs.getString("telnr"), rs.getString("email")));
    }
